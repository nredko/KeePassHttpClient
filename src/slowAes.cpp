...
* partially backported to C++ from JavaScript by nredko
*
...
void aes_main(uint8_t *state, uint8_t *expandedKey, int nbrRounds)
{
	int i = 0;
	uint8_t roundKey[16];
...
char aes_encrypt(uint8_t *input, uint8_t *output, uint8_t *key, enum keySize size)
{
	...
	case SIZE_32:
		...
		break;
	default:
		throw std::runtime_error("AES: UNKNOWN_KEYSIZE");
	}
	expandedKeySize = (16 * (nbrRounds + 1));
	if ((expandedKey = (uint8_t*) malloc(expandedKeySize * sizeof(char))) == NULL)
	{
		throw std::runtime_error("AES: MEMORY_ALLOCATION_PROBLEM");
	}
...
void aes_invRound(uint8_t *state, uint8_t *roundKey)
{
	invShiftRows(state);
	invSubBytes(state);
	addRoundKey(state, roundKey);
...
void aes_invMain(uint8_t *state, uint8_t *expandedKey, int nbrRounds)
{
	int i = 0;
	uint8_t roundKey[16];
	createRoundKey(expandedKey + 16 * nbrRounds, roundKey);
...
char aes_decrypt(uint8_t *input, uint8_t *output, uint8_t *key,	enum keySize size)
{
	/* the expanded keySize */
	int expandedKeySize;
	/* the number of rounds */
...
std::vector<uint8_t> getNextBlock(std::vector<uint8_t> bytesIn, size_t start, size_t end)
{
	if (end - start > 16)
		end = start + 16;
	return  std::vector<uint8_t>(bytesIn.begin() + start, bytesIn.begin() + end);
}


void padBytesIn(std::vector<uint8_t>& data) {
	size_t len = data.size();
	uint8_t padByte = 16 - (len % 16);
	for (size_t i = 0; i < padByte; i++) {
		data.push_back(padByte);
	}
}

void unpadBytesOut(std::vector<uint8_t>& data) {
	size_t padCount = 0;
	int8_t padByte = -1;
	size_t  blockSize = 16;
	...
	if (padCount > 0)
		data.erase(data.end() - padCount, data.end());
}
...
{
	keySize size = (keySize)key.size();
	if (iv.size() % 16)
	{
		throw std::runtime_error("AES: iv length must be 128 bits.");
	}
	...
	if (!bytesIn.empty())
	{
		for (size_t j = 0; j < ceil(bytesIn.size() / 16); j++)
		{
			size_t start = j * 16;
			size_t end = j * 16 + 16;
			if (j * 16 + 16 > bytesIn.size())
				end = bytesIn.size();
			byteArray = getNextBlock(bytesIn, start, end);
			if (mode == CFB)
			{
				if (firstRound)
				{
					aes_encrypt(iv.data(), output.data(), key.data(), size);
					firstRound = false;
				}
				else
					aes_encrypt(input.data(), output.data(), key.data(), size);
				for (size_t i = 0; i < 16; i++)
					ciphertext[i] = byteArray[i] ^ output[i];
				for (size_t k = 0; k < end - start; k++)
					cipherOut.push_back(ciphertext[k]);
				input = ciphertext;
			}
			else if (mode == OFB)
			{
			...
			else if (mode == CBC)
			{
				...
				firstRound = false;
				aes_encrypt(input.data(), ciphertext.data(), key.data(), size);
...
std::vector<uint8_t> decrypt(std::vector<uint8_t> cipherIn, AesMode mode, std::vector<uint8_t> key, std::vector<uint8_t> iv)
{
	...
	std::vector<uint8_t> output(cipherIn.size());
	std::vector<uint8_t> byteArray(16);
	std::vector<uint8_t> bytesOut;
	// char firstRound
	bool firstRound = true;
	if (!cipherIn.empty())
	{
		for (size_t j = 0; j < ceil(cipherIn.size() / 16); j++)
		{
			size_t start = j * 16;
			size_t end = j * 16 + 16;
			if (j * 16 + 16 > cipherIn.size())
				end = cipherIn.size();
			ciphertext = getNextBlock(cipherIn, start, end);
			if (mode == CFB)
			{
				if (firstRound)
				{
				...
				for (size_t k = 0; k < end - start; k++)
					bytesOut.push_back(byteArray[k]);
				input = ciphertext;
			}
			else if (mode == OFB)
			{
			...
			{
				aes_decrypt(ciphertext.data(), output.data(), key.data(), size);
				for (size_t i = 0; i < 16; i++)
